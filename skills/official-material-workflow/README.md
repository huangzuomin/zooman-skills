...
python3 "$CODEX_HOME/skills/.system/skill-installer/scripts/install-skill-from-github.py" \
  --repo huangzuomin/zooman-skills \
  --path skills/official-material-workflow \
...
cp -R ./official-material-workflow ~/.codex/skills/
```

安装后，目标目录通常是：

- Linux / macOS / UOS / 麒麟等类 Unix 系统：`~/.codex/skills/official-material-workflow`
- Windows：`%USERPROFILE%\\.codex\\skills\\official-material-workflow`
...
cp -R ./official-material-workflow ~/.claude/skills/
```
...
- Linux / macOS / UOS / 麒麟等类 Unix 系统：`~/.claude/skills/official-material-workflow`
- Windows：`%USERPROFILE%\\.claude\\skills\\official-material-workflow`
...
Copy-Item -Recurse -Force .\official-material-workflow "$env:USERPROFILE\.codex\skills\official-material-workflow"
```
...
- 如需保存文件但未指定路径，默认会写入 `~/official-material-workflow-output/`
...
如果已安装到 `~/.claude/skills/` 或项目内 `./.claude/skills/`，可在 Claude Code 中直接调用对应 skill，或按宿主支持情况让其自动匹配到机关材料、公文写作和审稿任务。
...
- 新闻报告默认目录：`~/official-material-workflow-output/news-reports/`
- 一般公文草稿默认目录：`~/official-material-workflow-output/drafts/`
- 同名 `Markdown` 与 `Word` 文件默认放在同一目录
- 用户任务生成的最终成稿默认采用 `YYYYMMDD-标题-vNN` 命名；如同一次任务还会生成离线提示词、导出说明等辅助文件，则在版本号后追加产物类型后缀，如 `-提示词`、`-说明`

命名示例：

- Markdown：`~/official-material-workflow-output/news-reports/20260404-当前热点新闻报告-v01.md`
- Word：`~/official-material-workflow-output/news-reports/20260404-当前热点新闻报告-v01.docx`
- 离线提示词：`~/official-material-workflow-output/drafts/20260404-专项检查通知-v01-提示词.md`

<a id="overview"></a>

## 三、项目概览

这是一个面向 agents、Codex、Claude Code 以及 WebUI / AnythingLLM / Qwen / Claude.ai 等提示词宿主的中文机关材料工作流仓库。它以 `prompts/` 为单一主源，统一生成：
...
本项目保留原有“成稿器”能力，同时新增专家工作流规则：模糊任务、高风险任务、领导决策、政策建议、舆情专报、公开/保密/合法性事项，优先输出任务简报、写作资料包、证据与口径说明、正式文稿草案、审稿报告和待核实清单。
...
- 任务简报：对模糊需求先判断目的、读者、文种候选、行文方向、决策需求和缺失信息
- 上游交接包：消费 `personal-kb-steward` 生成的 handoff、material-pack 和 evidence-pack，不直接扫描知识库
- 文种判断与路由：根据事项性质、行文方向、主送对象判断通知、报告、请示、函、纪要等文种
- 证据分级：按 E5-E0 区分法律法规、正式文件、内部台账、媒体材料、未核实线索和模型推测的可用边界
- 审稿门禁：按 S0/S1/S2 标记阻断项、人工确认项和可自动修正项
- 草案包输出：对高风险任务输出任务简报、写作资料包、证据说明、草稿、审稿报告和待核实清单
- 高阶材料框架：支持领导简报、改革方案、政策备忘录、项目报告、舆情专报、讲话稿的结构化判断
...
```bash
python3 renderers/docx.py \
  ~/official-material-workflow-output/drafts/20260404-专项检查通知-v01.md \
  -o ~/official-material-workflow-output/drafts/20260404-专项检查通知-v01.docx
```
...
```bash
python3 renderers/validate.py notice ~/official-material-workflow-output/drafts/20260404-专项检查通知-v01.md
```
...
- 任务简报、资料组织和草案包交付边界
- 上游 handoff / material-pack / evidence-pack 输入协议
- 证据分级、冲突处理和引用边界
- 审稿门禁、人工确认和待核实清单
- 高阶机关材料的决策框架
- Obsidian 元数据与归档规范
...
- [task-brief.md](./prompts/core/task-brief.md)：任务简报编译器规则。
- [upstream-handoff.md](./prompts/core/upstream-handoff.md)：与 `personal-kb-steward` 的上游交接包输入协议。
- [evidence.md](./prompts/core/evidence.md)：E5-E0 证据分级和冲突处理规则。
- [review-gates.md](./prompts/core/review-gates.md)：S0/S1/S2 审稿门禁规则。
- [advanced-frameworks.md](./prompts/core/advanced-frameworks.md)：领导简报、改革方案、政策备忘录、项目报告、舆情专报和讲话稿框架。
- [archive.md](./prompts/core/archive.md)：Obsidian 元数据、资料包和归档规范。
