...
class OfflineBuildTests(unittest.TestCase):
    ...
    def test_default_profile_contains_expert_workflow_rules(self) -> None:
        profile = load_profile("default")
        doc_types = sort_doc_types(load_doc_types(), profile.category_order)
        system_prompt = render_offline_system_prompt(profile, doc_types, None, include_examples=False)

        self.assertEqual(profile.skill_name, "official-material-workflow")
        self.assertIn("TaskBrief", system_prompt)
        self.assertIn("EvidenceCard", system_prompt)
        self.assertIn("ReviewReport", system_prompt)
        self.assertIn("official-material-handoff", system_prompt)
        self.assertIn("02-evidence-use-note", system_prompt)
        self.assertIn("E5/E4 可支撑正式判断", system_prompt)
        self.assertIn("S0：阻断成稿或发布", system_prompt)
    ...
    def test_small_local_prompt_uses_lite_shared_rules(self) -> None:
        ...
        self.assertNotIn("TaskBrief 固定字段", system_prompt)
        self.assertNotIn("ReviewReport 固定格式", system_prompt)
        self.assertNotIn("handoff.json 建议字段", system_prompt)
